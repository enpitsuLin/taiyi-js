...
import fp from './fast_parser';
import chain_types from './ChainTypes'
...
Types.asset = {
    fromByteBuffer(b) {
        ...
        if (precision >= 16) {
            ...
            let fai = new Buffer(b_copy.toBinary(), "binary").readInt32()
            fai = fai / 32
            symbol = "@@" + fai.toString().padStart(8, '0') + damm_checksum_8digit(fai).to_String()
            ...
            b.skip(7)
            // "1.000 YANG" always written with full precision
            amount_string = fromImpliedDecimal(amount, precision)
        }
    ...
    appendByteBuffer(b, object) {
        ...
        if (object["fai"]) {
            symbol = object["fai"]
            fai = parseInt(symbol.slice(2))
            let checksum = fai % 10
            fai = Math.floor(fai / 10);
            let expected_checksum = damm_checksum_8digit(fai)
    ...
    toObject(object, debug = {}) {
        if (debug.use_default && object === undefined) { return "0.000 YANG"; }
        return object
    }
}
...
Types.asset_symbol = {
    fromByteBuffer(b) {
        ...
        if (precision >= 16) {
            ...
            let fai = new Buffer(b_copy.toBinary(), "binary").readInt32()
            fai = fai / 32
            fai_string = "@@" + fai.toString().padStart(8, '0') + damm_checksum_8digit(fai).to_String()
            precision = precision % 16
            b.skip(3)
        }
        else {
            ...
            if (symbol == "YANG")
                fai_string = "@@000000021"
            else if (symbol == "QI")
                fai_string = "@@000000037"
            else
                throw new Error("Expecting non-smt core asset symbol, instead got '" + symbol + "'")
            b.skip(7)
        }

        return { "fai": fai_string, "precision": precision }
    },
    appendByteBuffer(b, object) {

        let fai = 0
        if (!object["fai"].startsWith("@@"))
            throw new Error("Asset Symbols NAIs must be prefixed with '@@'. Was " + object["fai"])

        fai = parseInt(object["fai"].slice(2))
        let checksum = fai % 10
        fai = Math.floor(fai / 10);
        let expected_checksum = damm_checksum_8digit(fai)
    ...
    toObject(object, debug = {}) {
        if (debug.use_default && object === undefined) { return "YANG"; }
        return object
    }
}
