...
import taiyiAuth from '../auth';
import { camelCase } from '../utils';

const debug = newDebug('taiyi:broadcast');
const noop = function() {}
const formatter = formatterFactory(taiyiApi);

const taiyiBroadcast = {};

// Base transaction logic -----------------------------------------------------

/**
 * Sign and broadcast transactions on the taiyi network
 */

taiyiBroadcast.send = function taiyiBroadcast$send(tx, privKeys, callback) {
  const resultP = taiyiBroadcast._prepareTransaction(tx)
    .then((transaction) => {
      debug(
      ...
      return Promise.join(
        transaction,
        taiyiAuth.signTransaction(transaction, privKeys)
      );
    })
    .spread((transaction, signedTransaction) => {
      ...
      return taiyiApi.broadcastTransactionSynchronousAsync(
        signedTransaction
      ).then((result) => {
...
taiyiBroadcast._prepareTransaction = function taiyiBroadcast$_prepareTransaction(tx) {
  const propertiesP = taiyiApi.getDynamicGlobalPropertiesAsync();
  return propertiesP
    .then((properties) => {
      // Set defaults on the transaction
      const chainDate = new Date(properties.time + 'Z');
      const refBlockNum = (properties.last_irreversible_block_num - 1) & 0xFFFF;
      return taiyiApi.getBlockHeaderAsync(properties.last_irreversible_block_num).then((block) => {
...
operations.forEach((operation) => {
  ...
  taiyiBroadcast[`${operationName}With`] =
    function taiyiBroadcast$specializedSendWith(wif, options, callback) {
      debug(`Sending operation "${operationName}" with`, {options, callback});
      const keys = {};
      if (operation.roles && operation.roles.length) {
        keys[operation.roles[0]] = wif; // TODO - Automatically pick a role? Send all?
      }
      return taiyiBroadcast.send({
        extensions: [],
  ...
  taiyiBroadcast[operationName] =
    function taiyiBroadcast$specializedSend(wif, ...args) {
      ...
      return taiyiBroadcast[`${operationName}With`](wif, options, callback);
    };
});

const toString = obj => typeof obj === 'object' ? JSON.stringify(obj) : obj;
broadcastHelpers(taiyiBroadcast);

Promise.promisifyAll(taiyiBroadcast);

exports = module.exports = taiyiBroadcast;
